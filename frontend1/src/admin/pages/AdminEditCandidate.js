...
const AdminEditCandidate = () => {
  ...
  useEffect(() => {
    const fetchCandidate = async () => {
      try {
        ...
        setFormData({
          ...
          electionCardNumber: candidate.electionCardNumber || '',
          electionCardPhoto: candidate.electionCardPhoto || '',
        ...
        // Store original photos to preserve them if not changed
        setOriginalPhotos({
          candidatePhoto: candidate.candidatePhoto || '',
          partySymbol: candidate.partySymbol || '',
          electionCardPhoto: candidate.electionCardPhoto || ''
        ...
        if (candidate.state) {
          const initialDistricts = getDistricts(candidate.state);
          setDistricts(initialDistricts);
          
          if (candidate.district) {
            const initialTalukas = getTalukas(candidate.state, candidate.district);
            setTalukas(initialTalukas);
            
            if (candidate.taluka) {
              const initialVillages = getPlaces(candidate.state, candidate.district, candidate.taluka);
    ...
    if (id) {
      ...
      const timer = setTimeout(fetchCandidate, 100);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Prepare data - only include photo fields if they've been changed
      const updateData = { ...formData };
      
      // Only send photo fields if they differ from originals (backned will preserve if not sent)
      if (updateData.candidatePhoto === originalPhotos.candidatePhoto) {
        delete updateData.candidatePhoto; // Don't send if unchanged
      }
      if (updateData.partySymbol === originalPhotos.partySymbol) {
        delete updateData.partySymbol; // Don't send if unchanged
      }
      if (updateData.electionCardPhoto === originalPhotos.electionCardPhoto) {
        delete updateData.electionCardPhoto; // Don't send if unchanged
      }

      await api.put(`/api/candidates/${id}`, updateData);
      toast.success('Candidate updated successfully');
      navigate('/admin/candidates');
    } catch (e) {
      const errorMessage = e.response?.data?.message || 'Failed to update candidate';
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      <div className="mb-6">
        ...
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Edit Candidate</h1>
      </div>
              ...
              <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 border-2 border-dashed border-gray-300 dark:border-gray-600">
                ...
                <div className="flex flex-col items-center justify-center">
                  <input
                    ...
                    className="hidden"
                    id="candidatePhotoInputEdit"
                  />
                  <label
                    htmlFor="candidatePhotoInputEdit"
